...
// RemoteAudioSource and its volume observers belong to the signaling thread.
// Keep the existing ADM/mixer path, including its echo-cancellation reference.
bool AudioTrack::set_playout_volume(double volume) const {
  if (!std::isfinite(volume) || volume < 0.0 || volume > 10.0) {
    return false;
  }
  return rtc_runtime_->signaling_thread()->BlockingCall([this, volume] {
